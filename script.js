...
function displayHero() {
    ...
    // Ambil maksimal 3 game yang punya isHot: true
    const hotGames = games.filter(g => g.isHot).slice(0, 3);
    ...
    hotGames.forEach((game, index) => {
        // Buat Slide
        sliderWrapper.innerHTML += `
            <div class="hero-banner" style="background-image: url('${game.image}')">
                <div class="hero-overlay">
                    <span class="hot-label"><i class="fa-solid fa-fire"></i> HOT PROJECT</span>
                    <h2 class="hero-title">${game.title}</h2>
                    <p class="hero-desc">${game.desc.substring(0, 150)}...</p>
                    <button onclick="showDetail(${game.id})" class="play-btn" style="width: fit-content; padding: 10px 30px; background: var(--accent); color: black;">
                        Lihat Detail
                    </button>
                </div>
            </div>
        ...
        // Buat Dot
        sliderDots.innerHTML += `<div class="dot ${index === 0 ? 'active' : ''}" onclick="goToSlide(${index})"></div>`;
    });

    startAutoSlide(hotGames.length);
}
...
function goToSlide(index) {
    currentSlide = index;
    updateSlider();
    // Reset timer kalau user klik manual
    const hotGamesCount = games.filter(g => g.isHot).slice(0, 3).length;
    startAutoSlide(hotGamesCount);
}
...
// Tambahkan panggil fungsi ini di paling bawah script.js
displayHero();
