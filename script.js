...
// Fungsi untuk membuat Tab A-Z secara otomatis
function initAZFilter() {
    const azTabs = document.getElementById('azTabs');
    if (!azTabs) return;

    // 1. Ambil semua huruf depan unik dari judul game, urutkan A-Z
    const letters = [...new Set(games.map(game => game.title.charAt(0).toUpperCase()))].sort();
    ...
    azTabs.appendChild(allBtn);

    // 2. Buat tombol untuk setiap huruf yang ada
    letters.forEach(letter => {
        const btn = document.createElement('button');
        btn.className = 'az-btn';
        btn.innerText = letter;
        btn.onclick = () => filterByLetter(letter, btn);
        azTabs.appendChild(btn);
    });
}

// Fungsi Filter berdasarkan Huruf
function filterByLetter(letter, btn) {
    ...
    if (letter === 'all') {
        ...
        const filtered = games.filter(game => game.title.startsWith(letter));
        renderGames(filtered);
    }
    
    // Reset filter kategori atas jika ada
    document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active'));
}
